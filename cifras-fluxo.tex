...
Neste capítulo, vamos apresentar uma forma de construir uma classe de sistemas que satisfazem essa definição, as {\em cifras de fluxo}.
...
Definimos um {\em Gerador de Números Pseudo-aleatórios} (GNP) como um algoritmo $G$ que recebe uma semente $s$ de um determinado tamanho $n$ e produz uma sequência maior de bits de tamanho $l(n)$.
...
Para que um GNP seja considerado seguro, a sequência gerada a partir da semente deve ser indistinguível de uma sequência verdadeiramente aleatória de mesmo tamanho ($r$).
...
Neste caso, vamos reduzir o problema de construir um sistema seguro contra ataques contra o texto cifrado apenas (ATC) ao problema de construir um GNP.
...
Neste contexto, se pudermos construir um GNP seguro, podemos utilizar esse GNP para construir um sistema de criptografia que seja seguro contra ataques ATC.
...
A {\em cifra de fluxo} é um método de criptografia que converte texto plano $m$ em texto cifrado $c$, utilizando um fluxo de chaves pseudoaleatórias gerado por um gerador de números pseudoaleatórios (GNP).
...
\begin{theorem}
  \label{theo:stream}
  Uma cifra de fluxo para uma mensagem de tamanho fixo $n$ que utiliza um gerador de número pseudoaleatório para expandir a chave é seguro contra ataques ATC.
\end{theorem}
\begin{proof}
...
\begin{center}
  \begin{tikzpicture}
    ...
    \node[above] at ($(large.north) + (0,2)$) {$\{0,1\}^{l(n)}$};
...
Em outras palavras, um GNP é uma função $G$ que, a partir de um elemento aleatório escolhido no conjunto $\{0,1\}^n$, produz deterministicamente um elemento no conjunto maior $\{0,1\}^{l(n)}$, onde $l(n) > n$. 
O gerador é considerado seguro se um adversário não for capaz de distinguir a saída de $G$ de uma escolha verdadeiramente aleatória feita diretamente no conjunto maior $\{0,1\}^{l(n)}$.
...
Analisaremos como essas definições mais fortes podem nos proteger contra adversários com capacidades adicionais e em diferentes cenários de ataque, como os ataques de texto plano escolhido (ETP) e ataques de texto cifrado escolhido (ETC).
...
Os {\em Registradores de Deslocamento com Retroalimentação Linear} (RDRLs) são uma classe de algoritmos frequentemente utilizados em criptografia para gerar sequências pseudo-aleatórias de bits.
...
Não demonstraremos formalmente este resultado. 
...
A escolha de um bom GNP é crucial para garantir a segurança de uma cifra de fluxo.
